...
select_all: async (req, res)=>{// Get all users
    try {
        let users = await admin_model.find().exec()
        res.send(users)
    } catch(err) {
...
select1_by_id: async (req, res)=>{// Get a selected user
    try {
        let user = await admin_model.findById(req.params.id).exec()
        res.send(user)
    } catch(err) {
        res.status(500).send(err)
    }
...
register: async(req, res)=>{// Save an user Record
    try {
        ...
        let user   = new admin_model(req.body)
...
authenticate: async(req, res)=>{// Check valid user or not
    try {
        console.log(req.body)
        let users = await admin_model.find({ password: req.body.password }).exec()
        //console.log(users.length)
        //console.log(users)
        if(users.length == 1) {// Found User record for given email_id & pass_word
            res.send(create_token(req.body.password))// Send token to Frontend if a Valid User is Logging in
            //res.send("Valid User")
        } else {
